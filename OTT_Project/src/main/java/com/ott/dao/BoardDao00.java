...
public class BoardDao00 extends DBConn {
    /**
     * Board-delete
     */
    /*
    ...
    public ArrayList<StarBoardVO> getMyPageReviewList(String id, int startRow, int endRow) {
        ArrayList<StarBoardVO> list = new ArrayList<StarBoardVO>();
        String sql = "select rno, rboard_num ,rboard_flat,rboard_title, avgscore, mem_id,rboard_date,rboard_hit from"
                + "(select rownum rno, rboard_num ,rboard_flat,rboard_title, avgscore, mem_id, to_char(rboard_date,'yyyy-mm-dd') rboard_date,rboard_hit"
                + "from (select rboard_num ,rboard_flat, rboard_title, (rboard_pic+rboard_money+rboard_quantity+rboard_kinds+rboard_div)/5 as avgscore,"
    ...
    /************************************************************************************************
     * Comment- select(Mypage-my comment) ///////수정중////
     */
    public ArrayList<CommentVO> myCommSelect(String id) {
        ArrayList<CommentVO> clist = new ArrayList<CommentVO>();
        String sql = "select c.comm_num, c.board_num, c.rboard_num, c.comm_contents, c.mem_id, to_char(c.comm_date, 'yyyy-mm-dd'), b.board_title, b.board_type, r.rboard_title"
                + " from ottcomment c, ottboards b, ottreviewboards r"
                + " where c.board_num=b.board_num or c.rboard_num=r.rboard_num and c.mem_id=?"
        ...
        try {
            ...
            while (rs.next()) {
                CommentVO vo = new CommentVO();
                vo.setNum(rs.getString(1));
                vo.setBnum(rs.getString(2));
                vo.setRnum(rs.getString(3));
                vo.setContent(rs.getString(4));
                vo.setId(rs.getString(5));
                vo.setDate(rs.getString(6));
                vo.setBtitle(rs.getString(7));
                vo.setBtype(rs.getInt(8));
                vo.setRtitle(rs.getString(9));
    ...
    /**
	 * 게시판 게시글 검색(안씀, 쿼리바꾸기전에 쓰던거라서 주석처리 요망. 삭제는 말아주셈)
	 */
/*	public ArrayList<BoardVO> select_search_freeboard(int board_type, String search_type, String text) {
		...
		try {
			BoardVO vo = new BoardVO();
			if(search_type.equals("mem_id")) {//mem_nick이 쿼리에 없기때문에 실행은 안됨. mem_id를 mem_nick으로 변환은 됨.
                String invert_mem_id_to_mem_nick = this.getnick(search_type);
                sql = "select rownum rno, board_num ,board_title, mem_id, to_char(board_date,'yyyy-mm-dd') board_date, board_good, board_bad, board_hit "
                        + " from (select board_num ,board_title, mem_id, board_date, board_good, board_bad, board_hit from ottboards "
                        + " where board_type= ? and " + invert_mem_id_to_mem_nick + " like ? order by board_date desc)";
			}
			pstmt.setInt(1, board_type);
...
//				close();
    ...
    public ArrayList<BoardVO> select_search_freeboard(int board_type, String search_type, String text) {
        //살려줘ㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓㅓ
        // board_type: 게시판 타입, search_type: 검색조건 ,text: 검색하려고 입력한 값, 
     
       ArrayList<BoardVO> olist = new ArrayList<BoardVO>();
       String sql =  "select rownum rno, ottboards.board_num ,ottboards.board_title,ottboards.mem_id, ottboards.board_date, ottboards.board_good, ottboards.board_bad, ottboards.board_hit, ottmember.mem_nick"  
               + " from ottmember, ottboards" 
               + " where ottboards.board_type= ?  and " +search_type+ " like '%' || "+  "'"+text+"'" + "|| '%' order by ottboards.board_date desc";
